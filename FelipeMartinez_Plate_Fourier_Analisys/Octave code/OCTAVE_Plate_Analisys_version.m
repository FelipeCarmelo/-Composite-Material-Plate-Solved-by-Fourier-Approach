%{
´◔‿ゝ◔`)━☞ANALISYS OF A PLATE EMBED IN THE BORDERS AND LOADED IN THE CENTRE
...
% Orientations
orientations = [60 -60 -60 60 60 -60 -60 60 60 -60 -60 60 60 -60 -60 60 60 -60 -60 60 60 -60 -60 60 60 -60 -60 60 60 -60 -60 60]
...
gravity = 9.81; % m/s2
Weigth = 0.2; % kg
load_surface = 200*200; % m2 "c"x"c"
p0=Weigth*gravity/load_surface;% MPa
 ...
 % ~~~~~~~~~~~ FINDING w(x,y), kx, ky, kxy ~~~~~~~~~~~~
    ...
    mn=1; %input('Define the iterations number: ');
    m=2*mn;   % m = rows     (i)
    n=m;    % n = colums  (j)
...
%mwxy_c;   mwxy_c=sum(mwxy_c,'all');     
wxy_c=mwxy_c*(16*p0*a^4)/(pi()^6)
%mkx;    mkx=sum(mkx,'all');             
kx=mkx*(16*p0*a^2)/(pi()^4)
%mky;    mky=sum(mky,'all');             
ky=mky*(16*p0*a^2*R^2)/(pi()^4)
...
%mkxy_00;   mkxy_00=sum(mkxy_00,'all');  
kxy_00=mkxy_00*(-32*R*a^2*p0)/(pi()^4)
%mwxy_00;   mwxy_00=sum(mwxy_00,'all');    
wxy_00=mwxy_00*(16*p0*a^4)/(pi()^6)
...
%mkxy_a0;   mkxy_a0=sum(mkxy_a0,'all');      
kxy_a0=mkxy_a0*(-32*R*a^2*p0)/(pi()^4)
%mwxy_a0;   mwxy_a0=sum(mwxy_a0,'all');      
wxy_a0=mwxy_a0*(16*p0*a^4)/(pi()^6)
...
%mkxy_0b;   mkxy_0b=sum(mkxy_0b,'all'); 
kxy_0b=mkxy_0b*(-32*R*a^2*p0)/(pi()^4)
%mwxy_0b;   mwxy_0b=sum(mwxy_0b,'all'); 
wxy_0b=mwxy_0b*(16*p0*a^4)/(pi()^6)
...
%mkxy_ab;   mkxy_ab=sum(mkxy_ab,'all');      
kxy_ab=mkxy_ab*(-32*R*a^2*p0)/(pi()^4)
%mwxy_ab;   mwxy_ab=sum(mwxy_ab,'all');      
wxy_ab=mwxy_ab*(16*p0*a^4)/(pi()^6)
...
%mkxy_ab;   mkxy_ab=sum(mkxy_ab,'all');      
kxy_D=mkxy_ab*(-32*R*a^2*p0)/(pi()^4)
%mwxy_c;   mwxy_c=sum(mwxy_c,'all');     
wxy_D=mwxy_c*(16*p0*a^4)/(pi()^6)
%mkx;    mkx=sum(mkx,'all');             
kxD=mkx*(16*p0*a^2)/(pi()^4)
%mky;    mky=sum(mky,'all');             
kyD=mky*(16*p0*a^2*R^2)/(pi()^4)
...
kxy_00 = sumkxs(kxy_00);kxy_a0 = sumkxs(kxy_a0);kxy_0b = sumkxs(kxy_0b);kxy_ab = sumkxs(kxy_ab);
wxy_00 = sumkxs(wxy_00);wxy_a0 = sumkxs(wxy_a0);wxy_0b = sumkxs(wxy_0b);wxy_ab = sumkxs(wxy_ab);
kxy_D = sumkxs(kxy_D);
...
disp('The Qx and Qy')
%mQx_xy=sum(mQx_xy,'all');  
 Qx_xy = mQx_xy*(16*a*p0/(pi()^3));
%mQy_xy=sum(mQy_xy,'all');  
 Qy_xy = mQy_xy*(16*a^2*p0/(b*pi()^3));
Qx_xy
Qy_xy
